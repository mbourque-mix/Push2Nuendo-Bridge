...
def build_release_notes():
    ...
    s.append(B("<b>macOS app is now fully self-contained</b>: the libusb USB runtime is bundled inside the .app, so Homebrew and <font face='Courier'>brew install libusb</font> are no longer required — just copy the app and run it. (The pre-built .app is Apple Silicon only; Intel Macs run from source — see the User Guide.)"))
    s.append(Paragraph("Bug Fixes", sH2))
...
def build_user_guide():
    ...
    s.append(Paragraph("Option A: Standalone App \u2014 Apple Silicon (Recommended)", sH2))
    ...
    s.append(Paragraph("The pre-built .app does not run on Intel Macs. Run from source instead (this also builds an Intel-native .app if you prefer \u2014 see the note at the end). Open Terminal:", sB))
    s.append(B("Step 1: Install Homebrew if needed \u2014 see https://brew.sh"))
    s.append(B("Step 2: Install libusb and Python 3.11 \u2014 <font face='Courier'>brew install libusb python@3.11</font>"))
    s.append(B("Step 3: From the project folder, install the dependencies:"))
    s.append(Paragraph("pip3.11 install -r requirements.txt<br/>pip3.11 install git+https://github.com/ffont/push2-python.git", sCode))
    s.append(B("Step 4 (optional, Plugin Mapper): <font face='Courier'>pip3.11 install fastapi uvicorn pedalboard</font>"))
    s.append(B("Step 5: Copy Ableton_Push2.js to the MIDI Remote folder (see Option A, Step 3)."))
    s.append(B("Step 6: Run the bridge \u2014 <font face='Courier'>cd src && python3.11 main.py</font>"))
    s.append(Paragraph("To build a standalone Intel .app on an Intel Mac, run <font face='Courier'>bash scripts/build.sh</font> (with the dependencies above installed). PyInstaller produces an architecture-native app, so building on Intel yields an Intel .app.", sNote))
    # 3
    ...
    s.append(B("<b>Double-click the .exe</b> to run the bridge \u2014 it starts in the <b>system tray</b> (icon near the clock), not a console window. Right-click the icon for status, Open Plugin Mapper (http://localhost:8100), Show Log and Quit. Run with <font face='Courier'>--terminal</font> for the classic console version."))
    ...
    s.append(B("Shift + press = open Edit Channel Settings for that track"))
...
def build_windows_install_guide():
    ...
    s.append(B("<b>Double-click the .exe.</b> The bridge starts in the <b>Windows system tray</b> (look for its icon near the clock, bottom-right). No console window opens."))
    s.append(B("<b>Right-click the tray icon</b> for the menu: connection status, <b>Open Plugin Mapper</b> (http://localhost:8100), <b>Show Log</b> (opens the log file), and <b>Quit</b>."))
    s.append(B("The bridge does <b>not</b> open a browser automatically. Use the tray menu's Open Plugin Mapper when you need it."))
    s.append(B("To <b>quit</b>, right-click the tray icon and choose Quit."))
    s.append(Paragraph("Prefer a console? Launch the .exe with the <font face='Courier'>--terminal</font> flag (e.g. from a Command Prompt, or a shortcut with --terminal appended) to get the classic console version with live status text.", sNote))
    ...
    s.append(B("The Plugin Mapper web server is bundled and starts automatically with the .exe — no extra install. Open it from the tray menu (<b>Open Plugin Mapper</b>) or browse to <font face='Courier'>http://localhost:8100</font> to create custom plugin parameter mappings. See the Plugin Mapper Guide for details."))
